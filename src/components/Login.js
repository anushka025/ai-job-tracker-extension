...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (signInError) {
      ...
      if (data.session && window.chrome?.runtime) {
        try {
          chrome.runtime.sendMessage(
            'gkobemhjaceoaldfcngmgiabmnnilici', // Your extension ID
            {
              type: 'SAVE_AUTH',
          ...
          console.log('⚠️ Extension messaging failed:', err.message);
        }
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Note: OAuth callback will handle token storage on redirect
  };
